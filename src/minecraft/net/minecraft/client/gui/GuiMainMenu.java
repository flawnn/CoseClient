...
import me.cose.functions.ScreenSettings;
import me.cose.functions.SoundPlayer;
import me.cose.gui.join.CoseJoinScreen;
import me.cose.utils.texture.GuiButtonExtended;
import me.cose.utils.texture.GuiScrollList;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback
{
    ...
    private GuiScrollList updateList;
    private Map<Integer, String> linksConnected;
    ...
    public GuiMainMenu()
    {
        ...
        try
        {
            ...
            if (!var2.isEmpty())
            {
                do
                {
                    this.splashText = (String)var2.get(rnd.nextInt(var2.size()));
                }
        ...
        this.updateCounter = rnd.nextFloat();
        this.field_92025_p = "";
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick)
    {
    	...
    	updateList.mouseClickMove(mouseX, mouseY);
    }
    
    protected void mouseReleased(int mouseX, int mouseY, int state)
    {
    	super.mouseReleased(mouseX, mouseY, state);
    	updateList.mouseReleased();
    }
    ...
    public void initGui()
    {
    	...
    	int sy = (int) ( height * 0.2);
    	int swidth = 80 * (int) (width / 427.0);
    	int sheight = 140 * (int) (height / 240.0);
    ...
    private void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_)
    {
        ...
        for(int i = 0; i <= Core.accounts.getMax(); i ++)
        {
        	...
        	String[] data = Core.accounts.getData(i);
        	int size = im.getWidth();
        	int left = nextLength - size;
        	nextLength = left - 4;
        	int id = 24 + i;
        	linksConnected.put(id, data[2]);
        	buttonList.add(new GuiButtonExtended(id, left, height - im.getHeight() - 4, im));
        }
        BufferedImage img = null;
        if(HeadManager.imgMainHead == null)
        {
        	try {
        		img = ImageIO.read(new URL(HeadManager.urlMainHead.replaceAll("%name%", mc.getSession().getUsername())));
        	} catch(Exception err) {}
        	if(img == null)
        		img = HeadManager.getNull(64, 64);
        	HeadManager.imgMainHead = img;
        }
        else
        	img = HeadManager.imgMainHead;
...
//        this.buttonList.add(this.field_175372_K = new GuiButton(14, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2, I18n.format("menu.online", new Object[0])));
    ...
    {
    	youtubeLink = url;
    	GuiConfirmOpenLink link = new GuiConfirmOpenLink(this, url, 29, true);
    	link.disableSecurityWarning();
    	mc.displayGuiScreen(link);
    }
    
    protected void actionPerformed(GuiButton button) throws IOException
    {
    	if(linksConnected.containsKey(button.id))
    	{
    		openLink(linksConnected.get(button.id));
    	}
    	
    	if (button.id == 20)
    	{
    		GuiButtonExtended btn = (GuiButtonExtended) button;
    		HeadManager.currentMainHead += Math.PI / 10;
    		btn.updateImage(TextureManager.rotation(HeadManager.imgMainHead, HeadManager.currentMainHead));
    	}
    	
    	if (button.id == 23)
    	{
    		this.mc.displayGuiScreen(new CoseJoinScreen(this));
//    		mc.displayGuiScreen(new ScreenSettings(this));
    	}
        ...
        else if (id == 20)
        {
        	if (result)
            {
                try
                {
                    ...
                    oclass.getMethod("browse", new Class[] {URI.class}).invoke(object, new Object[] {new URI(youtubeLink)});
                }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        GlStateManager.popMatrix();
        String var10 = Core.minecraftVersion;
        var10 += " <> Do not distribute!";
...
//        	mc.thePlayer = new EntityPlayerSP(mc, new WorldClient(mc.getNetHandler(), new WorldSettings(new WorldInfo(new WorldSettings(0, GameType.SURVIVAL, true, true, WorldType.CUSTOMIZED), "MyWorld")), 0, EnumDifficulty.PEACEFUL, mc.mcProfiler), mc.getNetHandler(), new StatFileWriter());
//        GuiInventory.drawEntityOnScreen(50, 75, 30, 50 - mouseX, 75 - 50 - mouseY, mc.thePlayer);
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        updateList.mouseClicked(mouseX, mouseY);
    }
    
    public void handleMouseInput() throws IOException
    {
    	super.handleMouseInput();
    	updateList.handleMouseInput();
    }
