...
public class GuiScreenResourcePacks extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        var1.updateRepositoryEntriesAll_Resource();
        ...
        List<ResourcePackListEntryHide> loadBehind = Lists.newArrayList();
        ...
        {
            var4 = (ResourcePackRepository.Entry)var3.next();
            if(Core.texture.getStopTexturePaths().contains(var4.getResourcePackName()))
            	loadBehind.add(new ResourcePackListEntryHide(this, var4));
            else
            	this.field_146969_h.add(new ResourcePackListEntryFound(this, var4));
        }
        
        for(ResourcePackListEntryHide hide : loadBehind)
        	this.field_146969_h.add(hide);
        ...
        if (button.enabled)
        {
            ...
            else if (button.id == 1)
            {
                ...
                {                	
                    ...
                    List<ResourcePackListEntryFound> loadBehind = Lists.newArrayList();
                    
                    while (var11.hasNext())
                    {
                        ResourcePackListEntry var13 = (ResourcePackListEntry) var11.next();
                        
                        if (var13 instanceof ResourcePackListEntryHide)
                        {
                        	var10.add(((ResourcePackListEntryFound) var13).func_148318_i());
//                        	loadBehind.add((ResourcePackListEntryHide) var13);
                        }
                        else if (var13 instanceof ResourcePackListEntryFound)
                        {
//                            var10.add(((ResourcePackListEntryFound) var13).func_148318_i());
                            loadBehind.add((ResourcePackListEntryFound) var13);
                        }
                    }
                    
                    for(ResourcePackListEntryFound found : loadBehind)
                    	var10.add(found.func_148318_i());
